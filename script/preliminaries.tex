...
\begin{tikzpicture}[->,>=stealth',auto,node distance=3.5cm,
  ...
  \node[main node, fill=green] (0) {$l_0$};
  \node[main node, fill=red] (1) [right of=0] {$l_1$};
  \node[main node, fill=blue] (2) [right of=1] {$l_2$};

  \node (4) [right of=2] {};
  \node (5) [left of=0] {};

  \path[every node/.style={font=\sffamily\small}]
    (0) edge[color=red!50!white] node {$t_1$} (1)
        edge[color=blue!50!white, bend right=25] node [below] {$t_2$} (2)
    (1) edge[color=red, loop above] node {$t_3$} (1)
        edge[color=blue!50!white] node {$t_4$} (2)
    (2) edge[color=blue, loop above] node {$t_5$} (2)
        edge node {$t_6$} (4)
    (5) edge[color=green!50!white] node {$t_0$} (0);
\end{tikzpicture}
...
\todo{Top-down vs bottom-up construction}
