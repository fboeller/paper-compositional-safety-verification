  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[->,>=stealth',auto,node distance=3cm,
      ...
      \only<2->{
        ...
        (1) edge[aligned edge, loop above, onslide={<5->scc1}] node {$t_3$\only<3->{$:$\\$i > 0 \wedge$\\$x' = x + 1 \wedge$\\$i' = i - 1$}} (1)
        (1) edge[aligned edge] node {$t_4$\only<3->{$: i = 0$}} (2)
        (2) edge[aligned edge, loop above, onslide={<5->scc2}] node {$t_5$\only<3->{$:$\\$x' = x + 1 \wedge$\\$y' = y + 1$}} (2)
