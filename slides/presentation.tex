...
\begin{frame}
  \frametitle{Programs}
  \begin{block}{Program}
    \only<2->{$\mathcal{L} = \lbrace \ell_0, \ell_1, \ell_2 \rbrace$}
    \only<3->{, $\mathcal{T} = \lbrace t_i \mid i \in \lbrace 1,\dots,6 \rbrace \rbrace$}
    \only<4->{, $\mathcal{V} = \lbrace x,y,i \rbrace$, $\mathcal{V}' = \lbrace x',y',i' \rbrace$}
  \end{block}
  \begin{figure}
    \centering
    \begin{tikzpicture}[->,>=stealth',auto,node distance=3cm,
      ...
      \only<2->{
        \node[main node, onslide={<6->scc0}] (0) {$l_0$};
        \node[main node, onslide={<6->scc1}] (1) [right of=0] {$l_1$};
        \node[main node, onslide={<6->scc2}] (2) [right of=1] {$l_2$};
      ...
      \only<3->{
        ...
        (0) edge[aligned edge] node {$t_1$\only<4->{$: x \geq y$}} (1)
        (0) edge[aligned edge, bend right=25] node [below] {$t_2$\only<4->{$: y > x$}} (2)
        (1) edge[aligned edge, loop above, onslide={<6->scc1}] node {$t_3$\only<4->{$:$\\$i > 0 \wedge$\\$x' = x + 1 \wedge$\\$i' = i - 1$}} (1)
        (1) edge[aligned edge] node {$t_4$\only<4->{$: i = 0$}} (2)
        (2) edge[aligned edge, loop above, onslide={<6->scc2}] node {$t_5$\only<4->{$:$\\$x' = x + 1 \wedge$\\$y' = y + 1$}} (2)
        (2) edge[aligned edge] node {$t_6$\only<5->{, assert($x \neq y$)}} (4)
        (5) edge[aligned edge] node {$t_0$\only<4->{$: i > 0$}} (0);
      }
    \end{tikzpicture}
  \end{figure}
\end{frame}
